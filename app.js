...
const slauteTextContainer = document.querySelector(".slauteTextContainer");
const saluteImgContainer = document.querySelector(".saluteImgContainer");
const jobTitelContainer = document.querySelectorAll(".jobTitelContainer");
...
window.addEventListener("scroll", () => {
  ...
  slauteTextContainer.style.transform = `translateY(${offset * 0.7}px);`;
  saluteImgContainer.style.transform = `translate(${offset * 0.4}px,${
    offset * 0.7
  }px)`;
  authorname.style.transform = `translateX(${offset * 0.1}px)`;
  jobTitelContainer[0].style.backgroundPositionY = `${offset * 0.5}px`;
  jobTitelContainer[1].style.backgroundPositionY = `${-offset * 0.4}px`;
  jobTitles[0].style.transform = `translateX(calc(200vh - ${offset}px))`;
  jobTitles[1].style.transform = `translateX(calc(-300vh + ${offset}px))`;
  projectsTitle.style.transform = `translateY(calc(400vh - ${offset}px))`;
  phones[0].style.transform = `translateX(calc(500vh - ${offset}px))`;
  laptop.style.transform = `translateX(calc(600vh - ${offset}px))`;
  phones[1].style.transform = `translateX(calc(750vh - ${offset}px))`;
});
